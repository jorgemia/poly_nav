...
int main(int argc, char ** argv)
{
  ...
  ros::init(argc, argv, "odom_pose_publisher");
  ros::NodeHandle nh;
  ...
  nh_priv.param<std::string>("map_frame",map_frame,"/odom");
  ...
  if(is_stamped)
    p_pub = nh.advertise<geometry_msgs::PoseStamped>("odom_pose", 1);
  else 
    p_pub = nh.advertise<geometry_msgs::Pose>("odom_pose", 1);
