...
int main(int argc, char ** argv)
{
  ...
  ros::init(argc, argv, "hokuyo_pose_publisher");
  ros::NodeHandle nh;
  ...
  nh_priv.param<std::string>("base_frame",base_frame,"/hokuyo");
  nh_priv.param<double>("publish_frequency",publish_frequency,10);
  nh_priv.param<bool>("is_stamped", is_stamped, false);

  if(is_stamped)
    p_pub = nh.advertise<geometry_msgs::PoseStamped>("hokuyo_pose", 1);
  else 
    p_pub = nh.advertise<geometry_msgs::Pose>("hokuyo_pose", 1);
